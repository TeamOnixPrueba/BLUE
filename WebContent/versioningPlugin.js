...
;(function() {
  ...
  $BLUEJQuery.extend(VersionBlue.prototype, {
	...
	createViewRelease: function(){
		...
		var plugin = this;
		var container = $BLUEJQuery("<div id='" + this.Id + "ReleaseContainer' class='bel-card' ></div>");
		var options = [];
	...
	createViewComponent: function(){
		...
		var container = $BLUEJQuery("<div id='" + this.Id + "ComponentContainer' class='bel-card margin-bottom-m' ></div>");
	...
	printChangeComponent: function(){
		...
		$BLUEJQuery.each(this.getChangeLogComponent($BLUEJQuery("#"+this.Id+"Components").val()), function (ite, change){
		 ...
		 CambiosList+= "</ul></div>";
		  $BLUEJQuery(containerChange).prepend("<div class='grid-row margin-bottom-s'><div class='col-12'><div class='bel-card'>"+Pase+Fecha+Version+CambiosTittle+CambiosList+"</div></div></div>");
		});
		    ...
		    $BLUEJQuery.each(release.Componentes, function (e, component){
                $BLUEJQuery.each(plugin.getChangeLogComponent(component.Nombre), function (e, changesComponent){
                    if(release.Pase==changesComponent.Pase){
					   tbody+="<tr><td><p class='bel-typography bel-typography-p'>"+changesComponent.Nombre+"</p></td><td><p class='class='bel-typography bel-typography-p''>"+
	...
	buildListIdStory: function (arrayChanges){
		...
		$BLUEJQuery.each(arrayChanges, function (e, changes){
			$BLUEJQuery.each(changes.historias, function (i, stories){
			   StoryIds+="<p class='bel-typography bel-typography-p display-block'>"+stories.id+"</p>";
			});
		});
		return StoryIds;
	},
